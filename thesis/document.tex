...
\title{Hydrological Modelling of the Sant Miquel Catchment: A Comparative Study of Physics-Based and Machine Learning Approaches}
\author{Name Surname}
\email{name@organization.com}
\tutors{Name Surname [and Name Surname]}
\specialization{Data Science}
\academicyear{2025/26}
\keywords{hydrological modelling, HEC-HMS, machine learning, flash floods}
...
catchment, Mallorca, Spain. The target is the water level recorded at station
STM08, in the Sa~Marjal wetland area at the catchment outlet into
s'Albufera, on a uniform 10-minute grid. A reproducible pipeline combines
historical in-situ hydrological observations, rain-gauge records from the
RiscBal and AEMET networks, and a production-database extension. The latter
required conversion of water levels from centimetres to metres, quality-aware
filtering, and a station-wise correction for anomalous diurnal sensor drift.
The resulting dataset contains 618\,769 timestamps, 614\,201 supervised
samples, and 255 predictor variables. Six baseline families are evaluated at
1-hour, 6-hour, and 24-hour horizons: persistence, ridge regression, random
forest, XGBoost, ARIMA, and ARIMAX. Persistence is the strongest aggregate
baseline, with test NSE values of 0.988, 0.913, and 0.563 at the three
horizons. Ridge is the strongest non-persistence model in aggregate, while
ARIMA and leakage-free ARIMAX provide more balanced long-horizon errors. The
HEC-HMS project has been initialized but not yet calibrated, and sequential
neural models have not yet been evaluated; therefore this version makes no
physical-versus-deep learning performance claim. It establishes the validated
data basis and benchmark results required for those subsequent phases.
\end{abstract}
...
storms and torrential runoff~\cite{Estrany2010,Estrany2020,Estrany2025}. Reliable
short-term water-level prediction is consequently relevant both to civil
protection and to the management of wetlands receiving intermittent catchment
flows.

The Sant Miquel catchment drains into s'Albufera, a coastal wetland in
northern Mallorca. The target station STM08 (UTM~zone~31N, 503\,205\,E,
4\,404\,521\,N) is located in the Sa~Marjal sector of this wetland, at the
catchment outlet. The catchment is instrumented by eight RiscBal hydrological
and meteorological stations and three AEMET rain gauges, but the data combine
different sampling frequencies, long sensor outages, quality flags, and
changes in the data acquisition system. These issues are not secondary
implementation details: the target is near zero for much of the record, while
the operationally important flood response occupies a small fraction of the
samples.
...
intended for a later phase~\cite{Breiman2001,Kratzert2019}.

The objectives of the present stage are fourfold: (i) to build a reproducible
10-minute dataset from heterogeneous observations; (ii) to correct and
document the centimetre-based production-database extension and its
suspicious records; (iii) to define a reproducible peak-over-threshold event
catalogue; and (iv) to establish a split-aware benchmark of six data-driven
baseline models. These results define the reference performance that future
physical and sequential models must exceed, particularly during flood events
and at long lead times.

\section{Study Area\label{sec:area}}

The Sant Miquel catchment is located in northern Mallorca, Spain, between the
Serra de Tramuntana mountain range and the s'Albufera wetland. The monitoring
station STM08 lies at 39.7906\textdegree{}N, 3.0374\textdegree{}E
(UTM~zone~31N, 503\,205\,E, 4\,404\,521\,N, 5.6~m a.s.l.). The catchment is
underlain primarily by Mesozoic karstic limestone, so part of the drainage
network is subsurface. Topography ranges from near sea level at the
Sa~Marjal wetland to more than 600~m in the headwaters, with steep channel
gradients.
...
The catchment outlet lies in the Sa~Marjal sector of s'Albufera, where the
target station STM08 is installed. Near-real-time observations are available
through the RiscBal hydrological-risk observatory of the Universitat de les
Illes Balears (\url{https://riscbal.uib.es}), which is the intended
operational context of the forecasting system.
...
rain gauges; Figure~\ref{fig:stations} shows their spatial arrangement. STM01
and STM02 are meteorological stations. STM03--STM08 are hydrological stations
measuring water level; discharge is derived from level through
station-specific rating curves. Water temperature is captured at the
...
\begin{table*}[!t]
    ...
    \begin{tabular}{@{}llllrcl@{}}
        ...
        15$\rightarrow$10$\rightarrow$5 min & 5.7 km WNW \\
        ...
        15$\rightarrow$10$\rightarrow$5 min & catchment outlet \\
        \midrule
...
STM08 is separated from the upstream hydrological cluster and receives flow
through the main channel and a direct tributary.

\begin{figure}[!b]
    \centering
    \includegraphics[width=\columnwidth]{station_map.png}
    \caption{Monitoring network and drainage network over the terrain of the
    Sant Miquel catchment. STM08 is the target station at the catchment
    outlet.}
    \label{fig:stations}
\end{figure}
...
Historical STM observations are supplied as source files, while the AEMET
rainfall series and the hydro-station extension are supplied as
station-specific production-database exports. Extension records are converted
to the common \texttt{HEIGHT\_m} schema before use.
...
2026-08-05. All timestamps are treated as Coordinated Universal Time (UTC).
The resulting uniform grid has 618\,769 timestamps, corresponding to
approximately 11.8 years at 10-minute resolution.
...
(not supplied for historical records). Production-database hydro
...
in the clean CSVs. Figure~\ref{fig:stm08} shows the resulting screened STM08
record over the full study period.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{stm08_overview.png}
    \caption{Screened STM08 water-level record and daily catchment
    precipitation. The series is highly intermittent, with isolated flood
    peaks separated by long near-zero periods, each preceded by rainfall
    accumulations.}
    \label{fig:stm08}
\end{figure*}
...
\begin{figure}[!b]
    ...
    and STM08. Markers indicate the maximum correlation and its lag for each
    upstream station, all at short routing lags.}
    \label{fig:ccf}
\end{figure}
...
mean. Peak magnitude and timing errors and exceedance metrics are also
calculated at 0.5 and 1.0~m. Results are reported for the complete
...
Figure~\ref{fig:baseline_nse} summarises the same comparison for the
validated portion of the test split.
...
non-persistence model by aggregate NSE, but its positive percent bias
(PBIAS) increases with lead time. At 24 hours, full-test PBIAS is 70.3\% for
ridge, 116.2\% for the
...
meteorological configurations, and a Java-based DSS interface. The catchment
...
The current project has reached a reproducible baseline stage. Correcting the
unit mismatch restored realistic extension levels and rating-curve discharges.
